...
For some reason tracks with zero popularity are over 10% of the dataset, which other than being suspicious is also VERY unbalanced. We decide to remove these observations as there seems to be too many of them, and also in some cases that the value doesn't make sense.
...
While handling and categorizing the genres we noticed some inconsistencies with the genre values across different songs. Some completely unrelated songs were wrongly categorized to the same genre, which makes sense since the genre is an algorithm-based variable so errors can happen. This leads us to think that we want to try a model without considering the genre, not even in the reduced form. Before proceeding let's see the average popularity of the new upper genres.
...
Despite all that, we are still curious to see the variation in the characteristics of songs across different years, and see what features of songs from different years contribute to its popularity right now. Let's see the mean of each variable as a function of the year.
...
Let's do the the same plot but for the correlation of each variable with the popularity over the years.
...
We hope that after building and analyzing the final model we will have more meaningful insights.

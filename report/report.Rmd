...
- Boxcox vs Qunatiles => Quantiles were better, only that is shown here
- Tried several transformations of perdictors (and response) based on the findings in the data part. Only the best version is shown here. 
...
check_model(model_add_quant_trans, quantile_data_tst)
...
We could get way closer.
quant and sqrt(pop) have the biggest effekt
either valence or loudness exp. Both make it worse.
exp(duration_m). 1/duration also possible but slighly worse.
Removing genre, is making it more supsect, adding upper_genre has no effect. So let's keep it at genre for now.
Adding year is making it more suspect

No visible improvements could be achieved towards the constant variance assumption.
...
TODO: Replace car library with faraway.

Using the model from before. Excluding upper_genre since it is a non specific subset for 
...
We see that the factor variable time_signature has vif values above the threshold of 5. This is problematic, it indicates that those variables are contributing to multicollinearity in our regression model. Therefore we remove them and do the test over again.
...
```{r}
check_model(model_add_quant_no_vif, quantile_data_tst)
plot_model(model_add_quant_no_vif)
```

TODO: Write text. Now all predictors are below the threshold.
No changes on the assumptions, neither good nor bad.

### Interaction

Let's see, if interactions are helpful
...
check_model(model_int_quant_trans, quantile_data_tst)
```
...
The model performs better in terms of meeting the normal assumption.
Also the fitted vs. residuals plot looks slightly better.
...
It's important, that year is excluded. Futher we would like to use upper_genre instead of genre.
The transformations are kept

```{r}
formula = "sqrt(popularity)~danceability+energy+exp(loudness)+speechiness+acousticness+instrumentalness+liveness+valence+tempo+exp(duration_m)+key+mode+genre"
model_simple = lm(formula, data = quantile_data_trn)
```
